# TODO: Translation updated at 2021-09-15 21:23

translate portuguese strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 9: Squizzle - Selfie"
    new "Dia 9: Squizzle - Selfie"

# TODO: Translation updated at 2021-09-15 21:23

translate portuguese strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Day 9: Squizzle - Selfie"
    new "Dia 9: Squizzle - Selfie"

# TODO: Translation updated at 2021-10-13 01:40

translate portuguese strings:

    # game/code/data/extended/extra_gallery.rpy:15
    old "Ask me one more time"
    new "Me pergunta mais uma vez"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Fill out the questionnaire"
    new "Preencha o questionário"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Been there done that"
    new "Já fiz tudo isso"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Complete the Week One Recap"
    new "Complete a Recapitulação da Primeira Semana"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Double Shower"
    new "Banho Duplo"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Take a double shower with Stacy"
    new "Tome um banho com Stacy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "How Bizarre, How Bizarre"
    new "Que bizarro, Que Bizarro"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Tell a very strange story while distracted"
    new "Conte uma história bem esquisita enquanto está distraído"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Now I've Seen Everything"
    new "Agora Eu Vi De Tudo"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Do all the Samiya's tasks to 100%"
    new "Faça todas as tarefas da Samiya."

    # game/code/data/extended/extra_gallery.rpy:15
    old "Fail Out"
    new "Reprovando"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Screw up on all of Samiya's tasks"
    new "Arruíne todas as tarefas da Samiya"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Motion Denied"
    new "Moção Negada"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Do not allow Lyssa to achieve completion before Min's party"
    new "Não permita que Lyssa termine antes da festa da Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Welcome to the party"
    new "Bem-vindo à festa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Play it safe"
    new "Vá com cuidado"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Scotty Doesn't Know"
    new "Scotty Não Sabe"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Choose and use an absurd safeword"
    new "Escolha eu use uma palavra de segurança absurda"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Daisy all the way"
    new "Daisy sempre"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Do everything you can with Daisy at Min's Party"
    new "Faça tudo que puder com Daisy na Festa da Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Battleaxe"
    new "Machado de Batalha"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Unlock a secret scene using Apples"
    new "Destrave uma cena secreta usando Maçãs"

    # game/code/data/extended/extra_gallery.rpy:15
    old "A walking simulator"
    new "Um simulador ambulante"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Fully explore the Proximity Event"
    new "Explore o Evento de Proximidade por completo"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Talk Your Ear Off"
    new "Falar até suas orelhas caírem"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Tell Lydia all the stories"
    new "Conte todas as histórias para Lydia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Outstanding Performance"
    new "Performance de Alta Qualidade"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Complete the Main Event at Min's party in every possible way"
    new "Complete o Evento Principal da festa da Min de todas as formas possíveis"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Min Max"
    new "Min Max"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Dump Squizzle for Min"
    new "Troque a Squizzle pela Min"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Now Serving Hot Pretzels"
    new "Agora servindo Pretzeis Quentes"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Get entangled with Polly and Nora"
    new "Brinque com Polly e Nora"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Coffee and a donut"
    new "Café e um donut"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Make Polly's coffee and give AmRose a donut"
    new "Faça o café da Polly e dê um donut para AmRose"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Doctor McSexy"
    new "Doutor McSexy"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Send a girl to the hospital"
    new "Mande uma garota para o hospital"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Death By Snu-Snu"
    new "Morte Por Snu-Snu"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Let Dahlia take you towards the light"
    new "Deixe que Dahlia o leve até a luz"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Hole for Glory"
    new "Buraco de Glória"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Watch adventures on MariaTV"
    new "Assista aventuras em MariaTV"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Blitzkrieg"
    new "Blitzkrieg"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Trigger two blitz challenges in one playthrough"
    new "Ative dois desafios blitz em uma campanha"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Who's Your Daddy?"
    new "Quem é o papai?"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Confirm that you will be a father"
    new "Confirme que você será pai"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Chasing Chastity"
    new "Procurando Chastity"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Get a special surprise from your girlfriend"
    new "Receba uma surpresa especial da sua namorada"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Eye for an eye"
    new "Olho por olho"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Make a trade with Terrell"
    new "Faça uma troca com Terrell"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Setec Astronomy"
    new "Setec Astronomia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Vanessa's little secret"
    new "Segredinho da Vanessa"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Play hockey with the Law"
    new "Jogue hockey com a Lei"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Do you have enough quarters?"
    new "Você tem moedas o bastante?"

    # game/code/data/extended/extra_gallery.rpy:15
    old "How’s that work?!"
    new "Como que isso funciona?!"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Deflower Cynthia's freshly renovated garden"
    new "Deflore o jardim recém-renovado de Cynthia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Boys are back in town"
    new "Os garotos estão de volta na cidade"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Spend some quality time with Oliver"
    new "Divirta-se um pouco com Oliver"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Why So Serious?"
    new "Por que tão sério?"

    # game/code/data/extended/extra_gallery.rpy:15
    old "View the trio of conflicts during the pool party"
    new "Veja os três conflitos durante a festa na piscina"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Consistent Bottom"
    new "Sempre Submisso"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Submit to everyone"
    new "Submeta-se a todo mundo"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Always On Top"
    new "Sempre no Topo"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Dominate everyone"
    new "Domine todo mundo"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Sleeping with the Fish"
    new "Durma com o Peixe"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Answering the Siren’s call"
    new "Respondendo ao chamado da Sereia"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Pretzel Mustard"
    new "Mostarda de Pretzel"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Be with as many people as possible in one playthrough"
    new "Fique com o máximo de pessoas possível em uma campanha"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Spicy Pretzel Mustard"
    new "Mostarda Apimentada de Pretzel"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Unlock all the people on the pool screen"
    new "Destrave todas as pessoas na tela da piscina"

    # game/code/data/extended/extra_gallery.rpy:15
    old "True Love, Part 2"
    new "Amor verdadeiro, Parte 2"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Show Stacy a lot of love!"
    new "Dê bastante amor a Stacy!"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Between the Thighs of an Angel"
    new "Entre as Coxas de um Anjo"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Fall asleep with Lydia in her bed"
    new "Caia no sono com Lydia na cama dela"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Due Diligence"
    new "Fazendo o Necessário"

    # game/code/data/extended/extra_gallery.rpy:15
    old "See both endings of Week 2"
    new "Veja ambos os finais da Segunda Semana"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Lowball"
    new "Lowball"

    # game/code/data/extended/extra_gallery.rpy:15
    old "Be with the fewest number of people by the end of the week"
    new "Tenha o mínimo número de pessoas no final da semana"

